...
工作流的种类决定它能用哪些节点类型：节点类型声明自己属于哪些种类，省略时为 `run`，因此第二种工作流出现之前写的节点照常可用，也不会误入 `code` 工作流。节点的种类不包含工作流的种类时保存被拒绝。种类出现之前保存的记录按 `run` 读回，因为写下它时只有这一种。
