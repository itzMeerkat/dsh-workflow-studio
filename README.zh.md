...
当 profile 带有 Harness 技能表时，本插件还注册 `workflow-code-atoms` 技能，正文是随包的 [`skills/workflow-code-atoms.md`](skills/workflow-code-atoms.md)。它指导模型把已有的 Go 代码拆成原子文件，用分支与合并把它们连起来，用 `create_workflow` 保存图，再用 `go vet` 检查写出的 `workflow.go` 并与原代码对照。
