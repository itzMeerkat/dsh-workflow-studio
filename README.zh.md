...
两种工作流都由同一个生成器写成源码：IR 决定执行顺序和每个节点所在的分支，工作流的语言决定块怎么开合、函数怎么声明、条件怎么取反，工作流的种类决定一个节点写成什么。`run` 工作流写成伪代码，节点写成对其类型的调用，数据边写成具名实参。`code` 工作流指定自己的语言——`python`、`typescript` 或 `go`——它的节点携带该语言的代码：`code-block` 是语句，原样写出并按所在的块重新缩进；`code-condition` 是一个表达式，写进读它的值的地方；`code-function` 是一个函数，在卡片上从文本文件中选择，参数和结果就是节点的输入和输出端口；指针参数或结果是可选端口，未接线的指针参数传入 `nil`。生成器只读函数的签名：函数写在文件顶层，节点在图中的位置写成一次调用，实参取自数据边，被读的结果在生成函数的开头声明，汇合函数结果的 `merge` 是各分支都赋值的同一个变量。`branch` 按连到其条件的值写成 if/else。目前只有 Go 能读函数签名，因此 Python 或 TypeScript 工作流中的 `code-function` 写不出来。语言是本包的代码，而不是插件注册的数据，因为浏览器在每次编辑时读函数签名来重画它的端口。

模型可以使用四个工具：

| 工具 | 用途 |
|---|---|
| `create_workflow` | 校验并持久化保存一个任一种类的具名工作流定义，以及 `code` 工作流的语言；每个 `code-function` 的端口按其签名读出 |
...
当 profile 带有 Harness 技能表时，本插件还注册 `workflow-code-atoms` 技能，正文是随包的 [`skills/workflow-code-atoms.md`](skills/workflow-code-atoms.md)。它指导模型把已有的 Go 代码拆成 `code-function` 原子，连接分支与合并，用 `create_workflow` 保存图，再把 `describe_workflow` 返回的源码与原代码对照。
...
侧栏中的 **Workflow Studio** 面板用于打开编辑器。所有操作集中在同一条工具栏上：作为面板标题就地编辑的当前工作流名称、可搜索的工作流选择器、视图标签页、标明来源插件的可检索节点菜单，以及刷新、导入、导出、保存和运行按钮。工作流 ID 由其名称派生，同时就是它的记录文件名，因此 `Daily Report v2` 存为 `daily-report-v2.json`；重命名会把记录换到新名称派生的 ID 下，重复名称会被拒绝。**导出** 按同一命名规则下载编辑器中的工作流。**导入** 把这样的文件读回编辑器，作为尚未保存的工作流，也接受直接从存储目录拷贝出来的记录文件；导入的工作流名称已被占用时会被改名，因此导入永不覆盖已保存的工作流。React Flow 画布为每个已声明输入和输出渲染一个连接点，输入位于左侧，输出位于右侧，每张卡片还带有 `run` 和 `then` 执行引脚。两个图绘制同一种卡片：节点名称，其下是节点类型与注册它的插件，再往下是端口与执行引脚、内联控件，以及最近一次运行产生了值的输出端口。节点 ID 用于错误信息和运行记录，而不占用卡片，因此改由详情面板显示。执行引脚只能连接执行引脚；分支节点按其声明的引脚各渲染一个输出引脚；`run` 引脚接受所有连到它的边，只要没有两条重复同一对引脚。拖动连线时，连接预览会跟随指针；已有边的端点可以移动到另一个兼容端口，也可以拖到画布空白处删除。选中节点后，其 ID、详情和输出会在全宽画布下方展开。两个边界节点各有自己的卡片。每一行为一个端口命名并选择类型，输入节点的行还可以填写默认值，重名或空名会被标记，因为边无法引用这样的端口。它们像其他节点一样拖动、保存和删除；新建的工作流自带这两个节点，因此声明输入的地方一开始就在画布上，而不是藏在节点库里，节点库也不会再提供第二个同侧节点。运行结束后，输出节点的卡片列出每个端口收到的值，也就是工作流交出的结果；没有值送达的端口不出现，而不是显示为 null，因此未执行的分支表现为缺一行。两个图都用这同一张卡片绘制边界节点，因此执行顺序视图会在阅读运行的地方给出运行结果；在那里它和该图中的其他卡片一样没有编辑控件。输出节点还带有 `run` 执行引脚：从分支或审批连进来的一条边决定工作流是否交出任何结果；输入卡片则提供 `then`，用于安排必须在输入就绪之后进行的工作。画布还支持节点定位、类型化端口连线、节点增删、卡片控件、卡片输出预览、JSON 配置编辑、坐标保存和运行状态覆盖。只读执行顺序视图使用按依赖深度排列的节点图替代原始 JSON 视图，每个阶段绘制为一条容纳本阶段卡片的泳道。它对数据依赖进行传递约简：如果另一条有向路径已经表示相同的执行顺序关系，就移除对应的直接边。执行边始终绘制，因为它由作者显式放置；由声明了多个引脚的节点引出的执行边标注所用的引脚名，例如 `true` 或 `false`。阶段表示依赖深度，而不是执行批次：每个节点在自身前驱全部结束后立即开始，因此一个长节点不会拖住与它无关的分支。运行列表会显示一次运行跳过了多少节点，因此悄悄走了分支的运行与全部执行的运行可以区分。侧栏为每种工作流各有一个入口——**Workflow Studio** 和 **代码工作流**，两者打开的是同一个编辑器：打开时的种类决定它列出和新建哪些工作流、节点库提供哪些节点类型，以及有哪些视图。只有 `run` 面板有 **运行** 和运行视图。两者都有 **源码** 视图，它把画布上的图（包括未保存的修改）写成工作流的语言：执行顺序自上而下阅读，分支写成 if/else。它与 `describe_workflow` 用的是同一个生成器。代码面板的工具栏选择这种语言；为 `code-function` 选择新文件会按函数签名重画它的端口，并去掉接在它不再声明的端口上的边；卡片显示文件的第一行和行数，而不是全文。面板只打开它能编辑的工作流：另一种类的记录或导入文件、有边的端点不是节点的工作流，或没有指定已知语言的代码工作流都会被拒绝，并用提示说明原因。画布下方的检查列表给出整图分析的结果，相关卡片上也带有标明严重程度和数量的标记。两者读的都是 Host 保存时运行的同一份分析，因此拒绝在按下保存之前就已可见；含有未知节点类型或环的图不做分析，因为这两件事已经分别由没有目录条目的卡片和执行顺序视图呈现。保存和运行操作通过 Host 的 `workflowStudio` Remote 完成，解析和图校验仍由 Host 统一负责。
...
| [`src/shared/language.ts`](src/shared/language.ts) | 各种语言、`code` 节点类型，以及按代码重读函数节点的端口 |
...
| [`src/client/WorkflowBoundaryCard.tsx`](src/client/WorkflowBoundaryCard.tsx) | 编辑边界节点所声明工作流端口的卡片 |
