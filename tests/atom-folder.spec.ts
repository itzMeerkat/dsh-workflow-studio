...
describe('原子目录', () => {
  it('只读该语言的文件，不含测试文件、生成的工作流文件和子目录；工作流读出其中的原子；子目录路径由 Host 拼接', async () => {
    ...
    try {
      ...
      assert.deepEqual((await readAtomFiles(folder, GO.functions!.atoms)).map(file => file.file), ['add.go'])
      ...
      assert.deepEqual(await listFolders(folder), {
        ...
        files: ['add.go', 'add_test.go', 'notes.md', 'workflow.go'],
      })
