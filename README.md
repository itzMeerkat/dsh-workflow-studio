...
The package's [`cordis.patch.yml`](cordis.patch.yml) inserts the `dsh-workflow-studio` plugin into a Harness profile. The plugin registers only the nodes the engine and the generator depend on — `branch`, `switch`, `merge`, the two boundary nodes, and the `code` nodes; node plugins supply the rest, such as the separate `dsh-workflow-demo-node` plugin with its agent-prompt, human-approval, and basic example nodes. The plugin requires `ctx.tools` and `ctx.storageDomain`, then provides `ctx.workflowNodeRegistry` and `ctx.dagEngine`. The base bundle supplies the JSON backend and routes domains to it.

A workflow has a kind, and the two kinds share everything except where they end up.

| Kind | What it is | What happens to it |
|---|---|---|
| `run` | Nodes that do work | The engine schedules and runs it |
| `code` | Nodes that carry code in the workflow's language | It is written out as one function in that language; it is never run |

A workflow's kind decides which node types it may use: a node type declares the kinds it belongs to and defaults to `run`, so a node written before the second kind existed keeps working and cannot appear in a `code` workflow by accident. Saving refuses a node whose kinds do not include the workflow's. A record saved before kinds existed reads back as `run`, because that is the only kind that existed when it was written. A saved workflow keeps its kind: updating it to the other kind is refused, and because both kinds share one name space, saving under a name the other kind already holds is refused instead of replacing that workflow. Only a `run` workflow can be started.

Both kinds are written out as source by one generator: the IR fixes execution order and which branch each node sits in, the workflow's language fixes how blocks open and close, how the function is declared and how a condition is negated, and the kind fixes what a node becomes. A `run` workflow is written as pseudocode, where a node becomes a call to its type with its data edges as named arguments. A `code` workflow names its language — `python`, `typescript` or `go` — and its nodes carry code in it: a `code-block` holds statements, written out unparsed and re-indented to the block it lands in, and a `code-condition` holds one expression, written wherever its value is read. A Go workflow also names an atom folder on the Host, which is one Go package: each `.go` file in it, other than `_test.go` files, the generated `*.workflow.go` files and `types.go`, exports exactly one function, named or bound to a `var`, and each such file becomes an atom the workflow can call; unexported helpers may sit beside that function, and `types.go` declares the custom types the atoms share and is never parsed. A `code-atom` node records only the file name, and its parameters and results are the node's input and output ports. A port's type is the exact Go type as written, such as `int`, `Order` or `*Coupon`, except that `float64`, `bool`, `string` and `any` (or `interface{}`) are the built-in port types `number`, `boolean`, `string` and `any`, so two ports connect only when their types are equal or one is `any`; a pointer parameter is an optional port, and an unwired one receives `nil`. The generator reads only an atom's signature and writes only the workflow's own function, in the folder's package: its parameters and results take the types the boundary ports declare, each atom node becomes a call whose arguments come from its data edges, each result something reads is declared at the top of the function, a `merge` of atom results is one variable every branch assigns, and the file imports only the atoms' imports whose package names the function uses. Saving a Go workflow that has an atom folder writes that function into the folder as `<id>.workflow.go`, named after the workflow's ID, beside the atoms and the other workflows' functions it calls, so `go build` compiles them together; renaming the workflow or changing its folder removes its previous file, and a workflow whose function cannot be written is still saved, its file removed, and the reason shown. A `branch` becomes `if`/`else` on the value wired to its condition. The languages are code in this package rather than plugin data, because the browser reads each atom's signature to draw its ports.

A `subworkflow` node embeds another saved workflow of the same kind as one node, linked by its ID in `config.workflow` rather than copied: its input ports are that workflow's inputs, where an input of a `run` workflow that has a default may stay unwired, and its output ports are that workflow's outputs. Like an atom node's, its ports follow the embedded workflow whenever the embedding one is opened or saved. The node menu adds one from its list of workflows, and the card's workflow selector links it to another, whose ports replace the old ones and whose missing ports take their edges with them. Saving refuses renaming a workflow that another one embeds, because the link is the ID its name derives. A `run` workflow expands each subworkflow when a run starts, from the embedded workflow's saved version: the node's ID becomes an entry that hands the values it receives, and the defaults of the inputs left unwired, to the embedded nodes, which are renamed `<node>/<id>` and each wait for the entry, so the whole subworkflow waits for its inputs and is skipped with its node. A node reading one of its outputs reads the embedded node that produced it, and an execution edge leaving it leaves the embedded workflow's output node. A run fails to start when a subworkflow does not exist, is of the other kind or embeds its own embedder, or when the expanded graph is invalid; the run record lists the expanded nodes. A `code` workflow writes a subworkflow as a call to the function the embedded workflow generates, so both must share a language that can call functions and an atom folder; a signature change reaches the embedding workflow's file when that workflow is saved again.

The model receives four tools:

| Tool | Purpose |
|---|---|
| `create_workflow` | Validate and durably save one named workflow definition of either kind, with a `code` workflow's language and atom folder; each `code-atom`'s ports are read from its atom's signature, and a Go workflow with an atom folder is written into it as `workflow.go` |
| `describe_workflow` | Write one saved workflow in its language — pseudocode for `run`, the language it names for `code` — with the analysis warnings it raises |
...
When the profile has the Harness skill registry, the plugin also registers the `workflow-code-atoms` skill, bundled as [`skills/workflow-code-atoms.md`](skills/workflow-code-atoms.md). It guides the model through splitting existing Go code into atom files, wiring them with branches and merges, saving the graph with `create_workflow`, and checking the written `<id>.workflow.go` with `go vet` and against the original.
...
Saving also runs a whole-graph analysis, which answers what no single rule can see: whether a data edge's source is guaranteed to have run whenever its target runs.
A required input whose source sits on a branch the target does not depend on is refused, because that wiring fails on every run that takes the other branch; the message names the execution pin to add.
Everything the analysis cannot prove is a warning instead — a `merge` whose sources may both deliver or neither, a workflow output with no value on some branch, a type the analysis narrowed that no longer fits its port.
Warnings never block a save, because the analysis does not know which branch a run will take; `create_workflow` and `describe_workflow` return them.
...
A node that branches declares its own execution pins through `execOutputs` and picks which fire by returning `next`; declaring pins replaces `then`, and omitting `next` fires all of them. The engine registers the node types whose behavior is execution semantics: `branch` takes a boolean `condition` and fires one of its `true` and `false` pins; `switch` takes a `value` and fires the pin of the case it equals, or `default` when it equals none, where its cases are the node's `config.cases`, each the name of its own pin, compared as text with a string, number or boolean value and edited on the card, whose renamed cases keep their edges; and `merge` is the only OR join — it runs when at least one incoming execution edge has fired, is skipped only when all are dead, and passes through the single input that arrived. Every other node is an AND join, so a node that must run regardless of which branch was taken sits downstream of a `merge` rather than downstream of either path.
...
The sidebar's **Workflow Studio** panel opens the editor. One toolbar carries all of it: the current workflow name, edited in place as the panel's title, then a searchable workflow picker, the view tabs, a searchable node menu whose rows identify their source plugins and which also lists the saved workflows of the same kind this one can embed, and the atoms, settings, refresh, import, export, save, and run actions. **Workflow settings** is a side panel, open by default, that holds everything belonging to the workflow rather than to one node: its description, a code workflow's language, and the inputs and outputs it declares, each a row with a name, a type and, for an input of a `run` workflow, a default value; the type list offers the built-in port types and every type the atoms use, spelled as the workflow's language writes them; a repeated or empty name is marked because an edge could not reference it, and a renamed or removed port takes its edges along. A workflow's ID is derived from its name and is also its record file name, so `Daily Report v2` is stored as `daily-report-v2.json`; renaming a workflow moves its record to the ID the new name derives, and a duplicate name is rejected. **Export** downloads the workflow open in the editor under that same derived file name. **Import** reads such a file back into the editor as an unsaved workflow, and also accepts a record file copied straight out of the storage directory; an imported workflow whose name is already taken is renamed, so importing never replaces a saved workflow. The React Flow canvas renders one handle per declared input and output, with inputs on the left and outputs on the right, plus the `run` and `then` execution pins on every card. Both graphs draw the same card: the node's name, then its type beside the plugin that registered it, then its ports and execution pins, its inline controls, and the output ports the latest run produced a value for. A node's ID identifies it in errors and run records rather than on its card, so the details panel carries it instead. An execution pin connects only to an execution pin, a branch node shows one output pin per pin it declares, and a `run` pin accepts every edge that reaches it as long as no two repeat the same pair. Connection previews follow the pointer while dragging. Existing edge endpoints can be moved to another compatible port or dropped on empty canvas space to delete the edge. Selecting a node opens its ID, its details, and its output below the full-width canvas. The declared inputs and outputs are the ports of two boundary nodes, which carry their own card: it lists the ports with their types and defaults and offers a handle per port, and the settings panel is where they are edited. The boundary nodes are dragged, saved and deleted like any other node, a new workflow starts with both of them, declaring a port adds back one the author deleted, and the node library stops offering a second one of either. After a run, the output node's card lists what reached each of its ports, which is what the workflow returned; a port nothing reached is absent rather than null, so a branch that did not run reads as a missing row. Both graphs draw the boundary nodes with that same card, so the execution-order view reports the run's result where the run is being read; there it has no editing controls, like every other card in that graph. The output node also takes a `run` execution pin: an edge into it from a branch or an approval decides whether the workflow returns anything at all, and the inputs card offers `then` for work that must be ordered after the inputs are in. The canvas also supports node placement, typed port-to-port connections, node creation and deletion, card controls, card output previews, JSON configuration editing, saved positions, and run-status overlays. Dragging a card's right edge sets its width, which is saved with the node when it differs from the default; the height follows the content, and a name or type too long for the card is cut short and shown in full on hover. The read-only execution-order view replaces the raw JSON view with a node graph arranged by dependency depth, where each stage is drawn as a band holding its own cards. It applies transitive reduction to data dependencies, removing a direct edge when another directed path already represents the same execution-order relation. An execution edge is always drawn, because the author placed it deliberately, and one leaving a node that declares several pins is labelled with the pin it uses, such as `true` or `false`. A stage is dependency depth rather than an execution batch: each node starts as soon as its own predecessors have settled, so one long-running node does not hold up an unrelated branch. The Runs list reports how many nodes a run skipped, so a run that quietly took a branch is distinguishable from one that ran everything. The sidebar carries one entry per workflow kind, **Workflow Studio** and **Code Workflows**, and both open the same editor: the kind it was opened with is passed to the Host's `snapshot`, which returns only that kind's workflows and the node types usable in them, so neither panel lists the other kind's workflows. The kind also decides which views exist and names a new workflow `workflow-N` or `code-workflow-N`. Only a `run` panel has **Run** and the Runs view. Both have the **Source** view, which writes the graph on the canvas, unsaved edits included, in the workflow's language: execution order reads top to bottom and a branch becomes `if`/`else`. It is the same generator `describe_workflow` uses. In a code workflow the settings panel picks that language, Go by default. A Go workflow also has the **Atoms** side panel, open by default on the left: it shows the atom folder and whether it holds `types.go`, its **Choose folder** button opens a browser over the Host's folders, and it lists the folder's atoms with their signatures, where clicking one adds a node that calls it, so the node menu offers registered node types only. It also lists the atom nodes whose file has left the folder, and each file that is not an atom with the reason. Its reload button, like **Refresh**, reads the folder again, moving atom nodes' ports when a signature changed and dropping the edges on ports it no longer declares. Cards show port types as the workflow's language writes them. Saving then reports that the function was written into the folder as `<id>.workflow.go`. A panel opens only a workflow it can edit: a record or imported file of the other kind, one with an edge whose endpoint is not a node, or a code workflow naming no known language is refused with a notice saying which. Below the canvas, the checks list reports what the whole-graph analysis found, and each affected card carries a marker giving its severity and count. Both read the same analysis the Host runs on save, so a refusal is visible before the save button is pressed; a graph holding an unknown node type or a cycle is not analyzed, because each is already shown by the card without a catalog entry and by the execution view. Save and run operations use the Host's `workflowStudio` Remote; parsing and graph validation remain Host-owned.
...
| [`src/flow-nodes.ts`](src/flow-nodes.ts) | The engine's `branch`, `switch`, `merge`, and boundary nodes, and the OR-join check |
...
| [`src/shared/analysis.ts`](src/shared/analysis.ts) | Whole-graph analysis: condition guards, reachability diagnostics, and output type inference |
| [`src/shared/ir.ts`](src/shared/ir.ts) | Lowering a definition to the IR: nodes in execution order, nested under the pins that guard them |
| [`src/shared/source.ts`](src/shared/source.ts) | Writing the IR in a language, and the faults a language cannot write |
| [`src/shared/language.ts`](src/shared/language.ts) | The languages, the `code` node types, atoms, and rereading an atom node's ports from its atom |
...
| [`src/subworkflow.ts`](src/subworkflow.ts) | Checking embeddings on save and expanding subworkflows when a run starts |
...
| [`src/client/WorkflowBoundaryCard.tsx`](src/client/WorkflowBoundaryCard.tsx) | The card that shows the workflow ports a boundary node declares |
| [`src/client/WorkflowSettings.tsx`](src/client/WorkflowSettings.tsx) | The workflow settings panel: description, language, inputs and outputs |
...
The model sees the `create_workflow`, `describe_workflow`, `run_workflow`, and `get_workflow_run` schemas and their rendered results. This package adds no system-prompt text and no runtime Skill.

### Token and cache effect

The four tool schemas increase every request that exposes the global tool set. Saved definitions and run records remain Host-side durable data; neither enters model context unless a tool result reports it.
