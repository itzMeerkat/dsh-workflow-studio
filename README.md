...
Both kinds are written out as source by one generator: the IR fixes execution order and which branch each node sits in, the workflow's language fixes how blocks open and close, how the function is declared and how a condition is negated, and the kind fixes what a node becomes. A `run` workflow is written as pseudocode, where a node becomes a call to its type with its data edges as named arguments. A `code` workflow names its language — `python`, `typescript` or `go` — and its nodes carry code in it: a `code-block` holds statements, written out unparsed and re-indented to the block it lands in; a `code-condition` holds one expression, written wherever its value is read; a `code-function` holds a function, chosen from a text file on its card, whose parameters and results are the node's input and output ports; a pointer parameter or result is an optional port, and an unwired pointer parameter receives `nil`. The generator reads only a function's signature: the function goes at the top of the file, the node's place in the graph becomes a call whose arguments come from its data edges, each result something reads is declared at the top of the generated function, and a `merge` of function results is one variable every branch assigns. A `branch` becomes `if`/`else` on the value wired to its condition. Only Go reads function signatures so far, so a `code-function` in a Python or TypeScript workflow cannot be written out. The languages are code in this package rather than plugin data, because the browser reads a function's signature on every edit to redraw its ports.

The model receives four tools:

| Tool | Purpose |
|---|---|
| `create_workflow` | Validate and durably save one named workflow definition of either kind, with a `code` workflow's language; each `code-function`'s ports are read from its signature |
...
| [`src/shared/language.ts`](src/shared/language.ts) | The languages, the `code` node types, and rereading a function node's ports from its code |
